 ...
 */
function domToReact(nodes, options, preserveCustomAttributes) {
  /**
  ...
  function skipAttributesToProps(node) {
    return (
      (preserveCustomAttributes ?? PRESERVE_CUSTOM_ATTRIBUTES) &&
      node.type === 'tag' &&
  ...
  var transform = options.transform || returnFirstArg;
  var replaceElement;
  var props;
  var children;
  ...
  for (var i = 0, len = nodes.length; i < len; i++) {
    ...
    if (skipAttributesToProps(node)) {
      ...
      props = attributesToProps(props, node.name, preserveCustomAttributes);
    }
